...
from apps.frontend.components.aqi_dashboard import render_aqi_dashboard, render_route_aqi_info
from apps.frontend.components.credit_card import (
    render_credit_comparison,
    render_credit_summary,
    render_grade_legend,
    render_segment_breakdown,
)
...
from apps.frontend.services.aqi_client import fetch_all_aqi
...
st.markdown(
    ...
    <div class="hero-card">
      <h2>🌱 EcoNav AI</h2>
      <p>Plan cleaner city routes • Real-time AQI • Earn Exposure Credits!</p>
    </div>
    """,
    unsafe_allow_html=True,
)
...
# --- Tabs ---
tab_route, tab_aqi = st.tabs(["🛤️ Eco Route + Credits", "🌍 Live AQI Dashboard"])
...
with tab_route:
    with st.form("eco_route_form"):
        ...
        submitted = st.form_submit_button("🚀 Find Eco Route & Calculate Credits", use_container_width=True)
            ...
            try:
                ...
                st.session_state.history.insert(
                    0,
                    {
                        ...
                        "credits": data.get("exposure_credits", {}).get("final_credit_change", "N/A"),
                        "grade": data.get("exposure_credits", {}).get("overall_grade", "?"),
                    },
                )
                ...
                if source == "real-time":
                    st.success("✅ Using **real-time AQI data** for route & credit calculations")

                # --- Exposure Credit Summary ---
                render_credit_summary(data, label="Eco Route")

                # --- Credit Comparison ---
                st.markdown("### ⚖️ Eco Route vs Shortest Route Credits")
                render_credit_comparison(data)
                ...
                if aqi_data:
                    render_route_aqi_info(aqi_data)

                # --- Segment Breakdown ---
                render_segment_breakdown(data)
...
with tab_aqi:
    ...
    try:
        aqi_response = fetch_all_aqi(api_url)
        render_aqi_dashboard(aqi_response)
    except RequestException as exc:
        st.error(f"Failed to fetch AQI data: {exc}")
